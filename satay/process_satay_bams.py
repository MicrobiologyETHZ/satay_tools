...
# def map_to_gff(gff_file, filtered_insertions_file):
#     """Map filtered files to GFF annotations."""
#     gff_name = Path(gff_file).stem

#     output_file = filtered_insertions_file.parent / \
#         f"{filtered_insertions_file.stem}_{gff_name}.cnts"
#     cmd = ["bedtools", "map", "-a", gff_file, "-b",
#            str(filtered_insertions_file), "-c", "1,4", "-o", "count,sum"]
#     try:
#         with open(output_file, "w") as out_file:
#             subprocess.run(cmd, stdout=out_file, check=True)
#         logging.info(
#             f"Mapped to GFF: {gff_file} with {filtered_insertions_file} -> {output_file}")
#     except Exception as e:
#         logging.error(
#             f"Error mapping to GFF for {filtered_insertions_file}: {e}")
#         raise
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Process samples for mutant analysis.")
    parser.add_argument("-s", "--samples", nargs="+", required=True,
                        help="List of samples to process.")
    parser.add_argument("--bam_directory", required=True,
                        help="Directory containing BAM files.")
    parser.add_argument("-o", "--output_directory", required=True,
                        help="Directory to store merged BAM files.")
    parser.add_argument("--interval_files", nargs="+",
                        required=True, help="List of interval files for mapping.")
    parser.add_argument("--gff_annotation_file", required=False,
                        default="/nfs/cds-peta/exports/biol_micro_cds_gr_sunagawa/scratch/Projects_NCCR/other/lheistinger/GCF_000146045.2.genes.gff",
                        help="Path to the GFF annotation file.")

    args = parser.parse_args()
    setup_logging(args.output_directory)
